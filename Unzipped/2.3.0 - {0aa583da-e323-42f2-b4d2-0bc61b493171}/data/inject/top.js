'use strict';
var dipWidth=Math.min(window.innerWidth*0.5,400); //width is hard coded to 500px, but no more than half of the window width
var dipHeight=Math.min(window.innerHeight-20,600);//height is hard coded to 700px, but no more than window height minus 20
...
function getURL(phrase) {
  var url="https://www.lexico.com/en/definition/"+phrase;
  return url;
}
  ...
  return {
    ...
    load: function() {
      iframe = Object.assign(document.createElement('iframe'), {
        ...
        style: `width: ${prefs.width}px; height: `+dipHeight+`px;`,
        src: 'about:blank'
      });
    ...
    hide: function() {
      if (iframe) {
        ...
        iframe.style.height = dipHeight+'px';
      }
    },
    width: function() {
      if (iframe) {
